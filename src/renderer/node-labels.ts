...
export function renderNodeLabels(ctx: GraphRenderContext, nodes: GraphNode[]): Selection<SVGTextElement, GraphNode, BaseType, unknown> {
  const selection = ctx.root
    ...
    .attr('font-size', 9)
    .attr('fill', (node: GraphNode) => node.style?.textColor ?? '#ffffff')
    .attr('pointer-events', 'none')
    .text((node: GraphNode) => node.label ?? node.id);

  selection.each(function (node: GraphNode): void {
    const textElement = this as SVGTextElement;
    const fullLabel: string = node.label ?? node.id;
    const radius: number = node.style?.radius ?? 8;
    const maxWidth: number = (radius * 2) - 6;
    let truncatedLabel: string = fullLabel;
