...
export function createGraph(config: GraphConfig): GraphInstance {
  try {
    ...
    const graphInstance: GraphInstance = {
      ...
      debugHover() {
        try {
          if ('debugHover' in v2Instance) {
            (v2Instance as any).debugHover();
          }
        } catch (error) {
          ErrorHandler.logError(error as Error);
        }
      ...
      // Debug method to export shadow canvas
      debugShadowCanvas() {
        try {
          if ('debugShadowCanvas' in v2Instance) {
            (v2Instance as any).debugShadowCanvas();
          }
        } catch (error) {
          ErrorHandler.logError(error as Error);
        }
