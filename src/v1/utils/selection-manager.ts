  ...
  private updateConnectedLinkPositions(nodeData: GraphNode): void {
    ...
    requestAnimationFrame(() => {
      ...
      this.root.selectAll<SVGLineElement, RenderableGraphLink>('line:not(.link-hit-area)')
        ...
        .each((item: RenderableGraphLink, _index, elements) => {
          const linkElement = elements[_index] as SVGLineElement;

          // Update link endpoints using the same logic as performance tick manager
          const sourcePoint = getShortenedSourcePoint(item.link, item.style, this.nodeRadiusCache || undefined);
          const targetPoint = getShortenedTargetPoint(item.link, item.style, this.nodeRadiusCache || undefined);
