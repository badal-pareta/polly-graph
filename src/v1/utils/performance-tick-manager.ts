...
import { NodeRadiusCache } from './node-radius-cache';
...
export class PerformanceTickManager {
  ...
  private nodeRadiusCache: NodeRadiusCache | null = null;
  ...
  /**
   * Set node radius cache for performance optimization
   */
  setNodeRadiusCache(cache: NodeRadiusCache): void {
    this.nodeRadiusCache = cache;
  }
  ...
  /**
   * Fast link position updates - now with proper source and target offsets and radius cache optimization
   */
  private updateLinkPositions(linkSelection: Selection<SVGLineElement, RenderableGraphLink, BaseType, unknown>): void {
    linkSelection
      .attr('x1', (item: RenderableGraphLink): number => getShortenedSourcePoint(item.link, item.style, this.nodeRadiusCache || undefined).x)
      .attr('y1', (item: RenderableGraphLink): number => getShortenedSourcePoint(item.link, item.style, this.nodeRadiusCache || undefined).y)
      .attr('x2', (item: RenderableGraphLink): number => getShortenedTargetPoint(item.link, item.style, this.nodeRadiusCache || undefined).x)
      .attr('y2', (item: RenderableGraphLink): number => getShortenedTargetPoint(item.link, item.style, this.nodeRadiusCache || undefined).y);
  }
