...
import {
  asyncWarmupSimulation,
  getOptimalWarmupTicks,
  getOptimalBatchSize
} from '../utils/async-warmup';
...
function getAdaptiveDefaults(nodeCount: number): AdaptiveDefaults {
  if (nodeCount < 50) {
    ...
    return {
      ...
      velocityDecay: 0.7,
      forces: {
        link: { strength: 0.4, distance: 150 },
        charge: { strength: -400 },
    ...
    return {
      ...
      velocityDecay: 0.75,
      forces: {
        link: { strength: 0.3, distance: 120 },
        charge: { strength: -300 },
        center: { strength: 0.03 },
        collide: { strength: 0.5, iterations: 1 }
      }
    };
  } else {
    return {
      ...
      velocityDecay: 0.8,
      forces: {
        link: { strength: 0.2, distance: 100 },
        charge: { strength: -250 },
        center: { strength: 0.02 },
        collide: { strength: 0.4, iterations: 1 }
      }
    };
  }
}
...
export function createGraphSimulation(
  ...
  if (enhancedConfig.warmup?.enabled !== false) {
    const warmupTicks = enhancedConfig.warmup?.ticks ??
      (useAdaptive ? getOptimalWarmupTicks(nodeCount) : 50);

    // Auto-enable async warmup for large graphs (backwards compatible)
    const shouldUseAsync = enhancedConfig.warmup?.async ?? (nodeCount > 200);

    if (shouldUseAsync && nodeCount > 200) {
      const batchSize = getOptimalBatchSize(nodeCount);
      asyncWarmupSimulation(simulation, {
        ticks: warmupTicks,
        batchSize,
        frameDelay: nodeCount > 500 ? 1 : 0
      });
    } else {
